...
1. On the **Logic** tab, drag the **ArrangeData** Server Action onto the flow.
    ...
    The **Grid** block receives data in JSON format. The **ArrangeData** Server Action (<ins>only</ins> available for **O11**) analyzes this data, serializes it, and retrieves the information from each column, whether it be in, for example, string, number, boolean format.
...
1. Set the **Data Action** property to the aggregate result. 
    ...
    **Note:** Because the **ArrangeData** Server Action action can receive any data structure, you must use the **ToObject** function. **ArrangeData** is <ins>only</ins> available for **O11**.
